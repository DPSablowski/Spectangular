...
void MainWindow::read_data(){
    ...
    if(ui->comboBox->currentIndex()==0){
        ...
        if(!checkfile3.exists()){
            ...
            QString fError= QString::number(g);
            QMessageBox::information(this, "Error", "Error 5: Spectrum "+qPath+"/"+input3+fError+".fits does not exist!");
            this->setCursor(QCursor(Qt::ArrowCursor));
            error=1;
           return;
        }
    ...
    if(!checkfile3.exists()){
        qDebug()<<"The file "<<checkfile3.fileName()<<" does not exist.";
        QString fError= QString::number(g);
        QMessageBox::information(this, "Error", "Error 6: Spectrum "+qPath+"/"+input3+fError+".txt does not exist!");
        this->setCursor(QCursor(Qt::ArrowCursor));
        error=1;
       return;
    }
    ...
    RV3a=RV3c[0];
    file1<<endl;
    file1<<dv<<" "<<RV1m<<" "<<RV3m<<" "<<RV1a<<" "<<RV3a<<endl;
...
void MainWindow::Optimisation()
{
    ...
    if(initstep.exists()){
        ...
        ifstream initStep("initialstep.dat");
        double IntS[8];
        string einse;
            ...
            if(res(i)>10*tsh){
                ...
                if(ui->checkBox_33->isChecked()){
                    if(ui->checkBox_14->isChecked()){
                        ...
                        if((abs(C(i)-res(i))<C(i)/3) & (W(i)>log10(ui->doubleSpinBox_14->value())) & (W(i)<log10(ui->doubleSpinBox_15->value()))){
                            ++resbins;
                            residu+=pow((res(i)-C(i)),2);
                        }
                    }
